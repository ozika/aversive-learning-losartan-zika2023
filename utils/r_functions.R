...
assign_var_types <- function(dfloc, fields) {
 ...
 # if ("drug_str" %in% fields) {dfloc$drug_str <- as.factor(dfloc$drug_str) }
  ...
  if ("sa" %in% fields) { 
        dfloc$sa <- dfloc$sa - mean(dfloc$sa)
  ...
  if ("ta" %in% fields) {
    ...
    dfloc$ta <- dfloc$ta - mean(dfloc$ta)
  ...
  if ("order_first" %in% fields) {
    dfloc$order_first <- as.factor(dfloc$order_first)
    
  }

  
  if ("order_second" %in% fields) {
    dfloc$order_second <- as.factor(dfloc$order_second)
    
  }
  
  if ("order_third" %in% fields) {
    dfloc$order_third <- as.factor(dfloc$order_third)
  ...
  if ("visit" %in% fields ) {
    dfloc$visit_str <- mapvalues(dfloc$visit,
                          from = c(1,2,3),
                          to = c("v1", "v2","v3"))
    dfloc$visit_str_nicer <- mapvalues(dfloc$visit,
                                 from = c(1,2,3),
                                 to = c("Visit 1", "Visit 2","Visit 3"))
